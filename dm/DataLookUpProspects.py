...
def ProHolders(tckr):
    data = requests.get(f"https://financialmodelingprep.com/api/v3/mutual-fund-holder/{tckr}?apikey="+ api_key)
    data = data.json()
    ...
    data = requests.get(f"https://financialmodelingprep.com/api/v3/institutional-holder/{tckr}?apikey="+ api_key)
    data = data.json()
    ...
    if df.empty:
        ...
        InstHolderQtrChg = df['change'].sum()
        InstHolderQtrChgPer = InstHolderQtrChg / (df['shares'].sum() - InstHolderQtrChg) # Change / previous quarter total
        
    return tckr, MutualFundQtrChg, MutualFundQtrChgPer, InstHolderQtrChg, InstHolderQtrChgPer
